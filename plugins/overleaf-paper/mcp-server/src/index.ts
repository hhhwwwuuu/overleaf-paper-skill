...
const server = new Server(
  { name: "overleaf-git", version: "0.1.0" },
  { capabilities: { tools: {} } },
);
...
function dirSchema(): Tool["inputSchema"] {
  return {
    type: "object",
...
async function withGitAuthEnv(): Promise<{ env: NodeJS.ProcessEnv; cleanup: () => Promise<void> }> {
  ...
  if (!isWindows) await chmod(askpass, 0o700);
  ...
  try {
    return await new Promise((resolvePromise, reject) => {
      ...
      child.on("close", (code) => {
        ...
        if (code === 0) resolvePromise(output || "OK");
